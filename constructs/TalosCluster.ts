...
export class TalosCluster extends Construct  {
    ...
    constructor(scope: Construct, name: string, props: TalosClusterProps ) {
        ...
        this.machineSecrets = new talos.machineSecrets.MachineSecrets(this, 'MachineSecrets').machineSecrets;
    
        this.talosConfig = new talos.clientConfiguration.ClientConfiguration(this, 'ClientConfiguration', {
          clusterName: this.clusterName,
          machineSecrets: this.machineSecrets,
        }).talosConfig;
        
        this.machineConfigurationControlPlane = new talos.machineConfigurationControlplane.MachineConfigurationControlplane(this, 'MachineConfigurationControlplane', {      
            clusterEndpoint: this.clusterEndpoint,
            clusterName: this.clusterName,
            machineSecrets: this.machineSecrets,
        }).machineConfig;

        this.machineConfigurationWorker = new talos.machineConfigurationWorker.MachineConfigurationWorker(this, 'MachineConfigurationWorker', {
            clusterEndpoint: this.clusterEndpoint,
            clusterName: this.clusterName,
            machineSecrets: this.machineSecrets,
        }).machineConfig;
    ...
    public addControlPlaneNode(name: string, endpoint: string, hostname: string): TalosNode {
        const appliedMachineConfiguration = new talos.machineConfigurationApply.MachineConfigurationApply(this, `ControlPlaneNode${name}`, {
            endpoint: endpoint,
    ...
    public getKubeConfig(node: TalosNode): string {
        return new talos.clusterKubeconfig.ClusterKubeconfig(this, `KubeConfig${node.name}`, {
            talosConfig: this.talosConfig,
            endpoint: node.appliedMachineConfiguration.endpoint,
            nodeAttribute: node.appliedMachineConfiguration.nodeAttribute,
        }).kubeConfig;
